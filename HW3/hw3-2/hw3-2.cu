...
//__host__ __device__ int ceil(int a, int b) { return (a + b - 1) / b; }
...
__global__ void phase3(int* d_dist, int r, int N) {
...
int main(int argc, char** argv) {
	...
	dim3 grid3(gridsize, gridsize);
	dim3 block(32, 32);

	for (int r = 0; r < gridsize; ++r) {
		
		phase1<<<1, block>>>(d_dist, r, N);
		phase2_1<<<grid2, block, 0, stream[0]>>>(d_dist, r, N);
		phase2_2<<<grid2, block, 0, stream[1]>>>(d_dist, r, N);
		phase3<<<grid3, block >>>(d_dist, r, N);
	}
